...
def main(n, m, k):
    ...
    model = rnn(vectorizer)
    model.compile(loss=tf.keras.BinaryCrossentropy(), optimizer=tf.keras.optimizers.SGD(), metrics=['binary_accuracy'])
...
def rnn(vectorizer, num_layers=1, emb_size=64, h_size=64):
